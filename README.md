...
**那两个 degradation 必须成对使用。** 首轮真实运行（`gemma3:4b`，[案例](docs/case_studies/gemma3_trajectory_case_study.md)）显示：同一条证据缺失，整步删除时评分**一分不降**，而保留调用、把结果换成「（无结果）」时降 1.07 分。前者暴露失效，后者是证明评分器并非迟钝的对照组。结论是评分器认的是**可见的失败标记**，不是证据是否存在——它会把结论里对缺失前提的复述当成支撑。这个探针在文本模态里没有对应物。

作弊变体只重复 Agent 真正做过的调用，**绝不凭空捏造工具名或调用结果**——那是伪造证据，不是构造变体。等义改写只在人工标注的分组内换序；没有标注就直接拒绝并指名案例，而不是硬造一个没人验证过的等义。

输出就是 `score` 的输入格式，随后的评分、审计、版本对比与来源标记全部沿用既有流程；生成清单额外记录 `modality: "trajectory"`。

配套的 `examples/trajectory_rubric.md` 有一个维度值得单独说：**佐证与稳健性**——关键事实是否经过独立的第二来源确认。它明写「重复同一调用不构成佐证」。有了这条，`redundant_tool_calls` 才是一个有意义的探针：评分器若把重复调用当成佐证加分，那是它的真实失效，而不是标准有歧义。

没有这一维时，一条结论正确、证据齐全、表述清楚的轨迹会锁死 8/10 分，作弊变体没有任何上升空间——**对一次本就完美的运行问「注水能不能加分」，等于问评分器会不会给出超过 10 分**。这是成对测试的固有性质，不是工具的缺陷；工具能做的是[检测并声明](#测量余量工具会说出自己测不了什么)该指标本次不可解读。
...
基准分贴近量表任一端时，对应方向的指标**不可解读**：

- 基准顶格（如 9.97/10）时，作弊变体没有上升空间。此时「作弊收益为零」无法与「评分器抵抗作弊」区分；
- 基准触底时同理，「退化降分不足」无法与「没有下降空间」区分。

声明了 `--score-min/--score-max` 后，审计会逐案例计算余量并在报告里写明，例如：

```text
- 量表余量：5/5 个案例的基准分距**量表上限**不足 1.00。对这些案例，
  作弊收益为零无法与「没有上升空间」区分，该指标不可解读。
```

这条来自实测教训：首轮轨迹审计中 `gemma4:e4b` 的基准均分是 9.97/10，「作弊对它无效」这个结论当时只能由我在案例散文里手写一句限制。散文会漏写，而读者看到的是数字，所以改由工具自己声明。余量按 `min_degradation_drop` 判定，不新增阈值——两个含义相近的阈值必然漂移，且漂移时无人察觉。

该声明**不改变风险等级**，只说明该指标本次无法解读。
